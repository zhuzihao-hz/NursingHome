...
public class PeopleSetInfoUIController implements Initializable {
    ...
    @FXML
    private JFXComboBox<Integer> peopleAgeComboBox;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        if (MANAGER_PRIV == 1) {
            ...
            peopleAgeComboBox.setDisable(true);
            peopleTypeComboBox.setDisable(true);
        }
    }
                ...
                if (peopleTypeComboBox.getValue().equals("护工")) {
                    ...
                    if (doorBoy.getWorkPlace().equals("前台")) {
                        ...
                        try {
                            Class.forName("com.mysql.jdbc.Driver");
                            conn = DriverManager.getConnection(MYSQL_URL, MYSQL_USER, MYSQL_PASSWORD);
                            String sql = "INSERT INTO NursingHome.administrator VALUES ('" + doorBoy.getId() + "',3,'" + md5(tempPassword) + "');";
                            stmt = conn.createStatement();
                            stmt.executeUpdate(sql);
                            stmt.close();
                            conn.close();
                    ...
                    if (N == 1) {
                        ...
                        try {
                            ...
                            if (admin.getPosition().equals("主管")) {
                                sql = "INSERT INTO NursingHome.administrator VALUES ('" + admin.getId() + "',0,'" + md5(tempPassword) + "');";
                            } else {
                                sql = "INSERT INTO NursingHome.administrator VALUES ('" + admin.getId() + "',1,'" + md5(tempPassword) + "');";
                            }
                            stmt = conn.createStatement();
                            stmt.executeUpdate(sql);
                            stmt.close();
